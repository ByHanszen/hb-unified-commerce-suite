...
class SubscriptionRepository {
    ...
    public function ensure_legacy_record_for_order($order): int {
        return $this->get_linked_legacy_post_id($order);
    }
    ...
    public function sync_legacy_from_order($order, bool $createIfMissing = false): ?array {
        ...
        $this->log_sync_debug('repository.sync_legacy_from_order.skipped_single_source_of_truth', [
    ...
    public function find(int $subscriptionId): ?array {
        ...
        if ($post instanceof \WP_Post && (string) $post->post_type === self::LEGACY_POST_TYPE) {
            $orderTypeRecord = $this->find_by_legacy_post_id($subscriptionId);
    ...
    public function get_order_type_subscription_data($order): array {
        ...
        $items = method_exists($order, 'get_meta') ? $order->get_meta(self::LEGACY_ITEMS_META, true) : get_post_meta($orderId, self::LEGACY_ITEMS_META, true);
        $feeLines = method_exists($order, 'get_meta') ? $order->get_meta(self::LEGACY_FEE_LINES_META, true) : get_post_meta($orderId, self::LEGACY_FEE_LINES_META, true);
        ...
        $hasItemsMeta = (method_exists($order, 'meta_exists') && $order->meta_exists(self::LEGACY_ITEMS_META))
            || metadata_exists('post', $orderId, self::LEGACY_ITEMS_META);
        $hasFeeLinesMeta = (method_exists($order, 'meta_exists') && $order->meta_exists(self::LEGACY_FEE_LINES_META))
