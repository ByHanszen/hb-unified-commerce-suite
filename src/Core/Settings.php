...
class Settings {
    ...
    public function init(): void {
        $this->cleanup_removed_checkout_module();
    ...
    private function defaults_subscriptions(): array {
        return [
            ...
            'frequencies' => [
                ...
                '7w' => [
                    'enabled' => 0,
                    'label'   => __('Elke 7 weken', 'hb-ucs'),
                    'interval'=> 7,
                    'period'  => 'week',
                ],
                '8w' => [
                    'enabled' => 0,
                    'label'   => __('Elke 8 weken', 'hb-ucs'),
                    'interval'=> 8,
                    'period'  => 'week',
                ],
            ],
    ...
    private function cleanup_removed_checkout_module(): void {
        $main = get_option(self::OPT, []);

        if (isset($main['modules']) && is_array($main['modules']) && array_key_exists('checkout', $main['modules'])) {
            unset($main['modules']['checkout']);
            update_option(self::OPT, $main, false);
        }

        if (get_option('hb_ucs_checkout_settings', null) !== null) {
    ...
    public function render_subscriptions(): void {
        ...
        foreach (['1w' => 1, '2w' => 2, '3w' => 3, '4w' => 4, '5w' => 5, '6w' => 6, '7w' => 7, '8w' => 8] as $key => $interval) {
            ...
            if ($rowLabel === '') {
                $rowLabel = $interval === 1
                    ? __('Elke week', 'hb-ucs')
                    : sprintf(__('Elke %d weken', 'hb-ucs'), (int) $interval);
            }
            ...
            echo '</label>';
            echo '<p class="description">' . esc_html(sprintf(__('Interval: %d weken (vast).', 'hb-ucs'), (int) $interval)) . '</p>';
            echo '</td>';
            echo '</tr>';
        }
